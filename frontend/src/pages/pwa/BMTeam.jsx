...
export default function BMTeam() {
  ...
  const myBDMs = useMemo(() => users.filter(u => u.role === "BDM" && u.parentUserId === user?.id), [users, user]);

  const getMemberStats = (bdm) => {
    const bdmCustomers = customers.filter(c => c.salesManagerId === bdm.id);
    const bdmBookings = bookings.filter(b => b.salesManagerId === bdm.id);
    return { customers: bdmCustomers.length, bookings: bdmBookings.length };
  };

  const handleAddBDM = (e) => {
    e.preventDefault();
    const form = e.target;
      const bdmCount = users.filter(u => u.role === "BDM").length + 1;
      const newBDM = {
        ...
        createdBy: user.id,
        status: "Active",
        employeeCode: `BDM-${String(bdmCount).padStart(3, '0')}`,
      };
    addUser(newBDM, user.id);
    setOpen(false);
    form.reset();
  };

  const toggleStatus = (bdm) => {
    updateUser(bdm.id, { status: bdm.status === "Active" ? "Inactive" : "Active" });
  };
  ...
  return (
    <div className="pb-24">
      <div className="px-4 py-4">
        ...
        <p className="text-sm text-gray-400 mt-0.5">Manage BDMs</p>
      </div>

      {/* Summary Cards */}
      <div className="px-4 grid grid-cols-2 gap-3 mb-4">
        {[
          { label: "Total BDMs", value: myBDMs.length, icon: Users, color: "text-blue-600", bg: "bg-blue-50" },
          { label: "Active", value: myBDMs.filter(b => b.status === "Active").length, icon: TrendingUp, color: "text-green-600", bg: "bg-green-50" },
          { label: "Customers", value: myBDMs.reduce((sum, b) => sum + getMemberStats(b).customers, 0), icon: Users, color: "text-purple-600", bg: "bg-purple-50" },
          { label: "Bookings", value: myBDMs.reduce((sum, b) => sum + getMemberStats(b).bookings, 0), icon: TrendingUp, color: "text-orange-600", bg: "bg-orange-50" },
        ].map(s => (
      ...
      <div className="px-4 space-y-3">
        {myBDMs.length === 0 ? (
          <div className="bg-white rounded-2xl p-8 text-center">
            ...
            <p className="text-gray-500">No BDMs yet</p>
          </div>
        ) : (
          myBDMs.map(bdm => {
            const stats = getMemberStats(bdm);
            return (
              <div key={bdm.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-gray-800 truncate">{bdm.name}</div>
                    <div className="text-xs text-gray-400 mt-0.5">{bdm.employeeCode} · {bdm.branch}</div>
                    <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                      <span className="flex items-center gap-1"><Mail size={12} />{bdm.email}</span>
                      <span className="flex items-center gap-1"><Phone size={12} />{bdm.mobile}</span>
                    </div>
                  </div>
                  <span className={`text-xs font-semibold px-2 py-1 rounded-full ${bdm.status === "Active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                    {bdm.status}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-50">
                  <button onClick={() => setSelectedMember(bdm)} className="flex-1 bg-gray-50 text-gray-600 py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1">
                    <Eye size={14} />View
                  </button>
                  ...
                  <button onClick={() => toggleStatus(bdm)} className={`flex-1 py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1 ${bdm.status === "Active" ? "bg-orange-50 text-orange-600" : "bg-green-50 text-green-600"}`}>
                    <Power size={14} />{bdm.status === "Active" ? "Disable" : "Enable"}
                  </button>
                </div>
              </div>
            );
        ...
        <div className="fixed inset-0 bg-white z-50">
          <div className="px-4 pt-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <h2 className="text-lg font-bold text-gray-900">Add Business Development Manager</h2>
          ...
          <form onSubmit={handleAddBDM} className="px-4 pb-6 space-y-4">
            <div>
            ...
            <button type="submit" className="w-full bg-blue-600 text-white py-3.5 rounded-xl font-bold hover:bg-blue-700 transition-colors shadow-lg shadow-blue-300">Create BDM</button>
          </form>
        </div>
      )}
