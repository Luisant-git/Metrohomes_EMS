...
import { ArrowLeft, User, Phone, Mail, Lock, MapPin } from "lucide-react";
...
export default function AddSalesManager() {
  const { user } = useAuth();
  const { addUser, users } = useData();
  const navigate = useNavigate();

  const allSMs = users.filter(u => u.role === "Sales Manager");
  const employeeCode = `SM-${String(allSMs.length + 1).padStart(3, '0')}`;
  ...
  return (
    <div className="pb-40 px-4 relative z-10">
      ...
      <div className="pt-4">
        ...
        <h2 className="text-2xl font-extrabold text-gray-900 mt-2 mb-3">Add Sales Manager</h2>
