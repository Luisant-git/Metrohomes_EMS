...
import { Users, TrendingUp, DollarSign, UserCheck, BarChart3, ArrowRight, UserPlus } from "lucide-react";

export default function BDMDashboard() {
  const { user, hierarchy } = useAuth();
  const { users, customers, visits, bookings } = useData();
  ...
  const myCustomers = customers.filter(c => c.salesManagerId && mySMs.some(sm => sm.id === c.salesManagerId));
  const myVisits = visits.filter(v => v.salesManagerId && mySMs.some(sm => sm.id === v.salesManagerId));
  const myBookings = bookings.filter(b => b.salesManagerId && mySMs.some(sm => sm.id === b.salesManagerId));
  ...
  return (
    <div className="pb-4">
      <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-700 text-white px-4 pt-4 pb-8">
        ...
        <div className="text-blue-200 text-sm mt-0.5">BDM · {user?.branch}</div>
      </div>
      ...
      <div className="px-4 -mt-5 space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => navigate("/add-sales-manager")}
              className="flex items-center gap-3 p-3 rounded-xl border-2 border-dashed border-blue-200 bg-blue-50/50 hover:border-blue-400 hover:bg-blue-50 transition-all text-left">
              <div className="w-9 h-9 bg-blue-600 rounded-xl flex items-center justify-center flex-shrink-0">
                <UserPlus size={18} className="text-white" />
              </div>
              <span className="text-sm font-bold text-blue-700">Add Sales Manager</span>
            </button>
            {quickActions.map(a => (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-bold text-gray-800 text-sm">My Sales Managers</h3>
            <button onClick={() => navigate("/my-team")} className="text-blue-600 text-xs font-semibold">View All</button>
          </div>
          <div className="space-y-2">
            {mySMs.length === 0 ? (
              <div className="text-sm text-gray-400 text-center py-4">No Sales Managers assigned</div>
            ) : (
              mySMs.slice(0, 3).map(sm => (
                <div key={sm.id} className="flex items-center gap-3 p-2 rounded-xl hover:bg-gray-50 transition-colors">
                  <div className="w-9 h-9 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-700 font-bold text-sm flex-shrink-0">{sm.name.charAt(0)}</div>
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-sm text-gray-800 truncate">{sm.name}</div>
                    <div className="text-xs text-gray-400 truncate">{sm.employeeCode}</div>
                  </div>
