...
export default function PMVisits() {
  const { user, hierarchy } = useAuth();
  const { visits, users, customers } = useData();
  ...
  const myVisits = visits.filter(v => {
    const salesManager = users.find(u => u.id === v.salesManagerId);
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              { label: "Scheduled", value: myVisits.filter(v => v.status === "Visit Scheduled").length, color: "text-orange-600" },
            ].map(s => (
