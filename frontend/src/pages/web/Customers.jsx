...
import { Eye, SquarePen, Trash2, UserCheck, Phone, UserPlus, AlertTriangle, Search, X, Calendar } from "lucide-react";
  ...
  const columns = [
    { key: "customerName", label: "Customer", render: (v, row) => (
      <div>
        ...
        <div className="text-xs text-gray-400 flex items-center gap-1"><Phone size={10} />{row.customer?.phone || ""}</div>
      </div>
    )},
    { key: "projectName", label: "Interested Project", render: (v, row) => `Site ${row.siteNo || "—"} - ${v || "—"}` },
    { key: "createdByName", label: "Created By", render: (v, row) => row.assignedToUser?.name || "—" },
    { key: "visitDateTime", label: "Scheduled Visit", render: (v, row) => `${formatVisitDate(row.visitDate)} ${formatVisitTime(row.visitTime)}` },
    { key: "status", label: "Current Status", render: v => <StatusBadge status={v} /> },
    { key: "createdAt", label: "Registered on", render: v => formatVisitDate(v) },
  ];
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-0.5">{siteVisits.length} total visits</p>
      </div>
      ...
      <div className="card p-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {(search || dateFrom || dateTo || filterStatus !== "All") && (
            <button onClick={() => { setSearch(""); setDateFrom(""); setDateTo(""); setFilterStatus("All"); }} className="px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors">
              Clear Filters
            </button>
          )}

          <span className="text-xs text-gray-400 ml-auto">{filteredVisits.length} of {siteVisits.length} visits</span>
        </div>
      </div>
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete Visit" size="sm">
        {deleteTarget && (
          <div className="text-center">
            ...
            <p className="text-sm font-medium text-gray-800 mb-4">
              ...
              <span className="block text-xs font-normal text-gray-400 mt-1">
                {formatVisitDate(deleteTarget.visitDate)} · {deleteTarget.projectName}
              </span>
            </p>
      ...
      {/* View Modal */}
      <Modal open={modal === "view"} onClose={() => setModal(null)} title="Visit Details">
        {selected && (
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-base font-extrabold text-gray-900 truncate">{selected.customer?.name}</h3>
      ...
      {/* Edit Status Modal */}
      <Modal open={modal === "edit"} onClose={() => setModal(null)} title="Update Visit">
        {selected && (
